...
- O estado do cursor (`_editText`, `_cursorPos`, `_atEnd`) vive no **`CursorController`** (`lib/ui/calculator/controllers/cursor_controller.dart`, Etapa 21); no modo de edição o `CalculatorViewModel` roteia a operação pelo editor e aplica o `EditorState` devolvido (`applyEditorState`) + `notifyListeners`
- `openParenCount` conta os parênteses do buffer de edição (`ExpressionEditor.countOpenParens`) quando o modo de edição está ativo — a lista de tokens confirmados do `ExpressionComposer` fica obsoleta nesse modo, e contá-la reportaria um balanço diferente do que o usuário vê
...
| Os tokens confirmados do `ExpressionComposer` ficam obsoletos quando o modo de edição é ativado | Estado derivado calculado sobre eles divergia do texto exibido (`openParenCount` reportava 0 com `(` na tela) | Todo estado derivado de parênteses lê o buffer de edição quando ele existe |
| Inserir `)` no fim do bloco numérico sem `(` pendente | Gerava `)` órfão (`12.50)`), tornando a expressão inavaliável e a prévia `null` | `ExpressionEditor.insertParenthesis` só fecha com `countOpenParens > 0`; caso contrário abre antes do bloco |
...
| A gravação da sessão era fire-and-forget | O processo podia morrer no meio da escrita, perdendo o `=` recém-pressionado | `SessionRecorder.persist()` devolve `Future<void>` e encadeia toda escrita em `pendingWrite`, que `flushSession()` aguarda |
| `add` em voo quando chega a 2ª linha da sessão | A linha era marcada como persistida sem nunca ser gravada | O `update` é encadeado no `Future` do `add` (`_pendingAdd`) e usa o id que ele devolve |
| `clear`/`loadSession`/paste com `add` em voo | O id da sessão antiga era adotado pela sessão nova | `startNewSession()`/`adoptSession()` incrementam a geração; o `add` só adota o id se a geração não mudou |
